<script>
    ...
    import { data, dmTranscripts, channelTranscripts } from '../app/store';
    import { generateAvatarURL, messageCountInfo } from '../app/helpers';
    ...
    onMount(() => {
        if (window.location.href.includes('/demo')) {
            ...
            const { dmTranscripts: demoTranscripts, channelTranscripts: demoChannelTranscripts, ...demoData } = generateDemoData();
            data.set(demoData);
            dmTranscripts.set(demoTranscripts);
            channelTranscripts.set(demoChannelTranscripts);
    ...
    // Message timestamps in the package are UTC. The persisted hoursValues
    // are bucketed in UTC by default; this picker recomputes hour buckets in
    // a different timezone from the in-memory transcripts for this session
    // (raw per-message timestamps aren't persisted to localStorage, so this
    // only works while the package is freshly loaded - not after a reload).
    ...
    try {
        if (typeof Intl.supportedValuesOf === 'function') {
            ...
            timezones = ['UTC', 'America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles', 'Europe/London', 'Europe/Paris', 'Europe/Berlin', 'Europe/Moscow', 'Asia/Tokyo', 'Asia/Shanghai', 'Asia/Kolkata', 'Australia/Sydney'];
        }
    } catch {
        // Leave the UTC-only fallback in place.
    }

    $: transcriptsLoaded = Object.keys($dmTranscripts).length > 0 || Object.keys($channelTranscripts).length > 0;

    const hoursForTimezone = (timeZone) => {
        const formatter = new Intl.DateTimeFormat('en-US', { timeZone, hour: 'numeric', hourCycle: 'h23' });
        const counts = new Array(24).fill(0);
        const allMessages = [
            ...Object.values($dmTranscripts).flatMap((t) => t.messages),
            ...Object.values($channelTranscripts).flatMap((t) => t.messages)
        ];
        for (let message of allMessages) {
            // hourCycle: 'h23' should give "0" for midnight, but some engines
            // inconsistently return "24" instead - wrap it back into range.
            counts[parseInt(formatter.format(new Date(message.timestamp)), 10) % 24]++;
        }
        return counts;
    };
    ...
    const rangeOf = (list) => {
        ...
        const counts = list.map((c) => c.messageCount);
        return [Math.min(...counts), Math.max(...counts)];
    };
    $: topDMsRange = rangeOf($data && $data.topDMs);
    $: topChannelsRange = rangeOf($data && $data.topChannels);
    $: allDMsRange = rangeOf($data && $data.allDMs);
    $: allChannelsRange = rangeOf($data && $data.allChannels);
    </script>

    <div class="statistics" transition:blur>
        {#if $data}
            <h1 class="packet-for">Packet for { $data.user.username }<small class="text-muted">#{ $data.user.discriminator.toString().padStart(4, '0') }</small></h1>
            <div class="cards">
                <CardGroup name="general-stats" title="General Stats">
                <Card name="profile">
                    <FunFact
                ...
                <Card name="hours" title="Your Discord Hours">
                    <div class="hours-header">
                        <div class="hours-timezone">
                            ...
                            {#if !transcriptsLoaded}
                                <small class="text-muted">(reload your package to change timezone)</small>
                            {/if}
                        </div>
                    </div>
                    <p>{ hoursLabels[displayedHoursValues.indexOf(Math.max(...displayedHoursValues))] } ({selectedTimezone}) is definitely your favorite hour to chat with your friends!</p>
                    <Chart data={{
                        labels: hoursLabels,
                        datasets: [
                            {
                                name: 'Messages',
                                values: displayedHoursValues
                            }
                        ]
                    }} axisOptions="{{
                        xAxisMode: 'tick'
                    ...
                    <Leaderboard description="The users you chat the most with!">
                        {#each $data.topDMs as channel, i}
                            <LeaderboardItem
                                ...
                                countColor={messageCountInfo(channel.messageCount, ...topDMsRange).color}
                                countRangeLabel={messageCountInfo(channel.messageCount, ...topDMsRange).rangeLabel}
                                countMessage={messageCountInfo(channel.messageCount, ...topDMsRange).message}
                                lastMessageAt={channel.lastMessageAt}
                                linkTo="/dm/{channel.id}"
                            />
                        {/each}
                    </Leaderboard>
                </Card>
                    ...
                    <Leaderboard description="The channels you chat the most in!">
                        {#each $data.topChannels as channel, i}
                            <LeaderboardItem
                                ...
                                count={channel.messageCount.toLocaleString('en-US')}
                                countColor={messageCountInfo(channel.messageCount, ...topChannelsRange).color}
                                countRangeLabel={messageCountInfo(channel.messageCount, ...topChannelsRange).rangeLabel}
                                countMessage={messageCountInfo(channel.messageCount, ...topChannelsRange).message}
                                lastMessageAt={channel.lastMessageAt}
                    ...
                    <Leaderboard description="Every user you've exchanged DMs with!">
                        {#each $data.allDMs as channel, i}
                            <LeaderboardItem
                                ...
                                discriminator={channel.userData.discriminator}
                                count={channel.messageCount.toLocaleString('en-US')}
                                countColor={messageCountInfo(channel.messageCount, ...allDMsRange).color}
                                countRangeLabel={messageCountInfo(channel.messageCount, ...allDMsRange).rangeLabel}
                                countMessage={messageCountInfo(channel.messageCount, ...allDMsRange).message}
                                lastMessageAt={channel.lastMessageAt}
                                linkTo="/dm/{channel.id}"
                            />
                        {/each}
                    ...
                    <Leaderboard description="Every server channel you've chatted in!">
                        {#each $data.allChannels as channel, i}
                            <LeaderboardItem
                                ...
                                guild={channel.guildName}
                                count={channel.messageCount.toLocaleString('en-US')}
                                countColor={messageCountInfo(channel.messageCount, ...allChannelsRange).color}
                                countRangeLabel={messageCountInfo(channel.messageCount, ...allChannelsRange).rangeLabel}
                                countMessage={messageCountInfo(channel.messageCount, ...allChannelsRange).message}
                                lastMessageAt={channel.lastMessageAt}
                                linkTo="/channel/{channel.id}"
                                channel
                            />
